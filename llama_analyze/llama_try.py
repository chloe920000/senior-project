...
* If it is bullish, usually the selling price will be higher than the buying price. If it is bearish, you do not need to answer question 2.3.4.5
...
2. Recommended buying price, considering a margin of error of +/- 5%?: [integer] NTD
3. Recommended selling price, assuming a stop-loss strategy with a maximum loss of 10%?: [integer] NTD
4. Recommended holding period for this investment? (months): [integer] months
...
'''
        ...
        # 將輸出存成txt檔案
        with open('output_2002.txt', 'w', encoding='utf-8') as f:
        ...
        # 解析輸出結果
        with open('output_2002.txt', 'r', encoding='utf-8') as f:
            output = f.read()
            parsed_result = parse_output(output)

        # 將結果寫入CSV檔案
        csv_file = 'output_2002.csv'
